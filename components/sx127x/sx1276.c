 ...
 */
void SX1276OnTimeoutIrq( TimerHandle_t );
 ...
 */
SX1276_t SX1276;
extern spi_device_handle_t radio_spi;
...
void SX1276Init( RadioEvents_t *events )
{
    ...
    // TODO: Replace with FreeRTOS software equivlent
    // TimerStop = xTimerStop / xTimerStopFromISR
    // TimerStart = xTimerStart / xTimerStartFromISR
    // TimerSetValue = xTimerChangePeriod / xTimerChangePeriod from ISR
    // TimerInit = xTimerCreate(pcTimerName, xTimerPeriod, uxAutoReload, pvTimerID, pxCallbackFunction)
    // TODO: Replace GPIO statements with ESP-IDF equilvalent
    // GpioInit() = gpio_config(pGPIOConfig)
    // GpioWrite() = gpio_set_level(gpio_num_t, uint32_t level)
    // GpioSetInterrupt() = gpio_config / gpio_isr_handler_add / gpio_install_isr_service

    // timer init
    TxTimeoutTimer = xTimerCreate("TxTimeoutTimer",1000, pdFALSE, (void *) 0, (TimerCallbackFunction_t) &SX1276OnTimeoutIrq);
    RxTimeoutTimer = xTimerCreate("RxTimeoutTimer", 1000, pdFALSE, (void *) 1, (TimerCallbackFunction_t) &SX1276OnTimeoutIrq);
    RxTimeoutSyncWord = xTimerCreate("RxTimeoutSyncWord", 1000, pdFALSE, (void *) 2, (TimerCallbackFunction_t) &SX1276OnTimeoutIrq);
    ...
    printf("Radio Version: %d", SX1276GetVersion());
    ...
    // enable TCXO
    uint8_t reg;
    reg = SX1276Read(REG_TCXO);
    printf("TXCO Reg: %x\n", reg);

    SX1276Write(REG_TCXO, (reg | BIT(4)));
    reg = SX1276Read(REG_TCXO);
    printf("TXCO Reg: %x\n", reg);
}
...
bool SX1276IsChannelFree( RadioModems_t modem, uint32_t freq, int16_t rssiThresh, uint32_t maxCarrierSenseTime )
{
    ...
    // TODO: determine if this is viable, explore if RTC is a good replacement
    //carrierSenseTime = TimerGetCurrentTime( );
    carrierSenseTime = xTaskGetTickCount() / portTICK_PERIOD_MS;
...
{
	// TODO: Confirm this operation is valid
...
void SX1276SetRx( uint32_t timeout )
{
    ...
    if( timeout != 0 )
    {
    	...
    	xTimerChangePeriod(RxTimeoutTimer, pdMS_TO_TICKS(timeout), 0);
    ...
    if( SX1276.Settings.Modem == MODEM_FSK )
    {
        ...
        if( rxContinuous == false )
        {
            ...
            xTimerChangePeriod(RxTimeoutSyncWord, pdMS_TO_TICKS(SX1276.Settings.Fsk.RxSingleTimeout), 0 );
...
void SX1276SetTx( uint32_t timeout )
{
	...
	xTimerChangePeriod( TxTimeoutTimer, pdMS_TO_TICKS(timeout), 0);
...
//TODO: Update to support ESP32-IDF SPI functionality
void SX1276WriteBuffer( uint16_t addr, uint8_t *buffer, uint8_t size )
{
	...
	t.cmd = 0;
	t.tx_data[0] = (addr | 0x80);				// write only command
	t.length = 8*1;
	t.flags = SPI_TRANS_USE_TXDATA;

	spi_device_transmit(radio_spi, &t);

    // write data
    for( i = 0; i < size; i++ )
    {
    	t.tx_data[0] = buffer[i];
    	spi_device_transmit(radio_spi, &t);
    }
...
//TODO: update to utilize ESP-IDF SPI buffers
//TODO: clear up the following code after the memset
void SX1276ReadBuffer( uint16_t addr, uint8_t *buffer, uint8_t size )
{
	...
	memset(&t, 0, sizeof(t));       // zero out the transaction
	t.cmd = 0;
	t.tx_data[0] = (addr & 0x7F);				// write only command
	t.length = 8*1;
	t.flags = SPI_TRANS_USE_TXDATA;
	spi_device_transmit(radio_spi, &t);
    ...
    switch( SX1276.Settings.State )
    {
        case RF_RX_RUNNING:
            ...
            case MODEM_FSK:
            	...
            	// TODO: Fix driver to properly utilize the SX1276 object
